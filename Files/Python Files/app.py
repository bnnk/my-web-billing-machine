...
def homepage_base(): 
    return '<a href = http://127.0.0.1:5000/buy>You will be redirected.Click Me in ROOT</a>'
...
def homepage_post():
    ...
    print('Points = %d' % cost)
    Text = "Received Post ["+request.form['choosen']+"] Cost is ["+str(cost)+"]"
    return render_template('task.html', txt = Text)
@app.route('/rev-project',methods=['GET','POST','PUT'])
def project():
    time.sleep(0.19)
    os.system('''"C:\Program Files (x86)\Google\Chrome\Application\chrome_proxy.exe" --profile-directory=Default --app-id=jljajlmgeemfjdgfmbhkdnejjpkohame''')
    return 'Starting app...'
